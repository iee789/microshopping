...
func IsPositive(m *pb.Money) bool {
	return IsValid(m) && m.GetUnits() > 0 ||
		(m.GetUnits() == 0 && m.GetNanos() > 0)
}
...
// 在没有错误时返回pb.Money对象，如果存在错误则触发panic
...
// 先验证输入的有效性，然后检查货币编码是否匹配，如果一切正常，它执行实际的加法运算
